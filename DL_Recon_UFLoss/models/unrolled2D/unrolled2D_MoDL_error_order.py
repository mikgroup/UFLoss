    ...
    def __init__(self, params):
        ...
        if params.error:
            ...
            self.conv_e = nn.Sequential(
                Conv2dIBNormRelu(4 * self.num_grad_steps, 64, 3, stride=1, padding=1,),
                Conv2dIBNormRelu(64, 64, 3, stride=1, padding=1,),
                Conv2dIBNormRelu(64, 32, 3, stride=1, padding=1,),
                Conv2dIBNormRelu(32, 32, 3, stride=1, padding=1,),
                Conv2dIBNormRelu(32, 16, 3, stride=1, padding=1,),
                Conv2dIBNormRelu(16, 16, 3, stride=1, padding=1,),
                Conv2dIBNormRelu(
                    16, 2, 1, stride=1, padding=0, with_ibn=False, with_relu=False,
                ),
            )
        else:
            UNet_output = 2
        ...
        if share_weights:
            ...
            self.unets = nn.ModuleList(
                [nn.ModuleList([UNet(2, UNet_output), self.CGM])] * self.num_grad_steps
            )
        else:
            print("No shared weights")
            self.unets = nn.ModuleList(
                [
                    nn.ModuleList([UNet(2, UNet_output), self.CGM])
        ...
        for i, unet in enumerate(self.unets):
            ...
            if self.error:
                if i == 0:
                    ...
                    image_all = torch.cat((image_all, image), 1)

        #             pl.ImagePlot(image.detach().cpu().numpy())
        if self.error:
            error_est = self.conv_e(torch.cat((error_all, image_all), 1))
            # print(torch.cat((error_all, image_all), 1).shape)
        # image = self.complex2real(image)
